...
*/
   ...
   //conditions:
      ...
      static boolean inSummary = false; static Pokemon summaryPoke; //static ImageIcon summaryPoke;
   ...
   //battle info
      ...
      private static Pokemon[] normalParty = new Pokemon[6]; // to save the order of your party before the battle starts (so you can switch around all you want)
    ...
    //extras:
       ...
       public class RefreshListener implements ActionListener
      {
          public void actionPerformed(ActionEvent e)
         {
            ...
            {
               bgColor = Color.WHITE;
               ImageIcon poke = new ImageIcon("Images/Pokemon/" + summaryPoke.getName() + ".png");
            ...
            //stats:
               myBuffer.drawString("Attack:", 10, 150);	 myBuffer.drawString("" + summaryPoke.getAttackLevel(), 70, 150);
               myBuffer.drawString("Defense:", 10, 170);	myBuffer.drawString("" + summaryPoke.getDefenseLevel(), 70, 170);
               //hp
               ...
               myBuffer.fillRect( 23, 60, (int)(100.0*summaryPoke.getCurrentHP()/summaryPoke.getMaxHP()), 6);
               //exp
               myBuffer.setColor(Color.BLACK);
            	//myBuffer.drawString("EXP: ",
               myBuffer.setColor(Color.BLUE);
               myBuffer.fillRect( 23, 80, (int)(100.0*(summaryPoke.getMyEXP() - summaryPoke.getMyPastLevelEXP())/(summaryPoke.getMyNextLevelEXP() - summaryPoke.getMyPastLevelEXP())), 5);
               myBuffer.drawString( summaryPoke.getMyEXP() + " out of " + summaryPoke.getMyNextLevelEXP(), 23, 100);
            }
            else if(canMove)//!inBattle)
            {
               ...
               if(map[curR][curC].isGate())
               {
                  ...
                  try
                  {
                     ...
                     {
                        System.out.println("Error: uploadMap() has failed in GameScreen @ RefreshListener");
                     }
                  curR = newR;
                  curC = newC;
               }
               ...
               if(map[curR][curC].getType() == Terrain.HEALING_TILE)
               {
                  for(int i = 0; i < 6; i++)
                  {
                     if(PlayPanel.myPokemon[i] !=null)
                        PlayPanel.myPokemon[i].heal( PlayPanel.myPokemon[i].getMaxHP() - PlayPanel.myPokemon[i].getCurrentHP());
                  }
               }
            ...
            //draw bg and stuff
                     ...
                     if(r >= curR - 5 && curR <= curR+5)
                        if(c >= curC - 8 && curC <= curC + 8)
                        {
                           if(map[r][c].getType()!=Terrain.GROUND)
                              myBuffer.drawImage(ImageLibrary.ground.getImage(), (c - curC + 6)*23 - SPRITE_WIDTH/2, (r - curR + 4)*26 - SPRITE_HEIGHT/2, null);
               ...
               //buildings and stuff
            	//people
            //YOU! lol
               myBuffer.drawImage(curSprite.getImage(), WIDTH/2 - SPRITE_WIDTH/2, HEIGHT/2 - SPRITE_HEIGHT/2 - 5, null); // -5 for visual purposes
            }
            else if(inBattle)
            {
               bgColor = Color.WHITE; 
            	...
            	//sprite of pokemon:
               myPokeIcon = new ImageIcon("Images/Pokemon/" + myPoke.getName() + "Back.png");  
            ...
            //me:   
               ...
               myBuffer.fillRect(myX + 23, myY + 23, (int)((100)*(1.0*myPoke.getCurrentHP()/myPoke.getMaxHP())), 7);
            
            //enemy:
               myBuffer.setColor(getPokemonHealthBarColor(enemy));   
               myBuffer.fillRect(enemyX + 23, enemyY + 23, (int)( (100)*( 1.0*enemy.getCurrentHP()/enemy.getMaxHP() ) ) , 7);
            ...
            //EXP Bar
               ...
               myBuffer.fillRect(myX+23,myY+40,(int)(100.0*(myPoke.getMyEXP() - myPoke.getMyPastLevelEXP())/(myPoke.getMyNextLevelEXP() - myPoke.getMyPastLevelEXP())),5);
            ...
            //textbox:
               ...
               else if(isAttacking)
               {
                  ...
                  {//gives the attack
                     ...
                     {
                        AttackLibrary.fill();
                        Attack a = new Attack(myAttackName,AttackLibrary.getType(myAttackName),10,AttackLibrary.getAttackDamage(myAttackName));
               ...
               {   
                  if(enemyMove == 1)
                  {
                     ...
                     //b = new Attack(enemyAttackName,AttackLibrary.getType(enemyAttackName),10,AttackLibrary.getAttackDamage(enemyAttackName));
                  }
                  ...
                  else if(enemyMove >= 11)
                  {
                     ...
                     if(accuracy > 5)
                     {
                        Attack b = new Attack(enemyAttackName,AttackLibrary.getType(enemyAttackName),10,AttackLibrary.getAttackDamage(enemyAttackName));
       ...
       public void saveParty() // save the party before the start of a battle
      {
         for(int i = 0; i < normalParty.length; i++)
            normalParty[i] = PlayPanel.myPokemon[i];
      }
      
       public void resetParty() // returns the battle party to its original order (you can switch out pokemon during a battle, but the order doesn't change)
      {
         for(int i = 0; i < normalParty.length; i++)
      ...
      {         
         ...
         {
            JOptionPane.showMessageDialog(null, "How in the world were you allowed into the wild with no usable pokemon???\nNow go rant the game makers.");
            System.exit(0);
         }   
         ...
         enemy  = new Pokemon("Raikou","Electric","Tackle","Shock","Thunder","Headbutt", 50,0, 55,30, 150, 150); //default
         setRandomEnemy();
         enemyIcon = new ImageIcon("Images/Pokemon/"+enemy.getName()+".png");
       ...
       public String[] loadEnemyArray() //Naveen
      {
         ...
         try
         {
            infile = new Scanner( new File("enemies.txt") );
         }
       ...
       public static void uploadMap(String name) throws IOException
      {
         ...
         BufferedReader in = new BufferedReader(new FileReader( new File("Maps/" + name) ) );
