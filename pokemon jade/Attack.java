    public class Attack
   {
       ...
       public Attack(String n,String t,int pp, int damageAmount)
      {
       ...
       // public static int getAttackDamage(Pokemon user, Pokemon opponent, int moveDamage){
         // double damage = ((((((user.getLevel() * 2 / 5) + 2) * moveDamage * user.getAttackLevel()) / 50) / opponent.getDefenseLevel()) + 2);
       ...
       public void giveDamage(Pokemon user, Pokemon victim, int attackDamage)
      {
         ...
         // double damage = ((((((user.getLevel() * 2 / 5) + 2) * damageOfAttack * user.getAttackLevel()) / 50) / victim.getDefenseLevel()) + 2);
         // if(Math.random() < 0.1) //critical hit
            // damage *= 2;
         // damage *= getRandomNumber() / 2; //random modifier x = [85, 100] 
         // 
      ...
      // 	
      	...
      	//(((((((level * 2 / 5) + 2) * basepower * atk / 50) / def) * mod1) + 2) * CH * mod2 * R/100)*STAB*type1*type2*mod3);
